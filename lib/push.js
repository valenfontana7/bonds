...
export async function sendDigestToDevice(deviceId, appUrl, { force = false } = {}) {
  ...
  const birthdays = subscriber.upcomingBirthdays ?? [];
  const hasBirthdayAlert = birthdays.some((entry) => entry.daysUntil <= 1);

  if (people.length === 0 && !hasBirthdayAlert && !force) {
...
export async function runScheduledDigests(env = process.env) {
  ...
  for (const subscriber of subscribers) {
    ...
    if (people.length === 0 && !hasBirthdayAlert) {
      skipped += 1;
      continue;
    }
    ...
    try {
      const payload = buildPushPayload(people, getPublicAppUrl(env), birthdays);
      await sendPushToSubscriber(subscriber, payload);

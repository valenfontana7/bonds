...
export async function runScheduledDigests(env = process.env) {
  ...
  const digestHour = Number.parseInt(env.DEFAULT_DIGEST_HOUR ?? '9', 10);
  const subscribers = await readSubscribers();
  let sent = 0;
  let skipped = 0;
  ...
  for (const subscriber of subscribers) {
    ...
    const hour = currentHourForTimezone(timezone);
    const targetHour = subscriber.digestHour ?? digestHour;
